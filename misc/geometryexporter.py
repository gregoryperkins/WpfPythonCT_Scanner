...
class Geometryexporter(Base):
	...
	def cmdReadyDelayTimerSlot(self):
		try:
			...
			print(e)
	def cmdExportPositions(self, filename):
		try:
			parameters = [filename]
			self.executeCommand(self.objectName, "exportPositions", parameters)
		except ApiError as e:
			print(e)
	def cmdExportPositionsFileDialog(self):
		try:
			self.executeCommand(self.objectName, "exportPositionsFileDialog")
		except ApiError as e:
			print(e)
