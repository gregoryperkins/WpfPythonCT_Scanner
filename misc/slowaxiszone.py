...
class Slowaxiszone(Base):
	...
	def cmdReadyDelayTimerSlot(self):
		try:
			...
			print(e)
	def cmdCompile(self, code):
		try:
			parameters = [code]
			self.executeCommand(self.objectName, "compile", parameters)
		except ApiError as e:
			print(e)
	def cmdUpdateAxisSpeed(self, axisPosition):
		try:
			parameters = [axisPosition]
			self.executeCommand(self.objectName, "updateAxisSpeed", parameters)
		except ApiError as e:
			print(e)
