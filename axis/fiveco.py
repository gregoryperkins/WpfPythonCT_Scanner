...
class Fiveco(Base):
	...
	def cmdDestroy(self):
		try:
			...
			print(e)
	def cmdClientInitialized(self, initState):
		try:
			parameters = [initState]
			self.executeCommand(self.objectName, "clientInitialized", parameters)
		except ApiError as e:
			print(e)
	def cmdEndPositionRequest(self):
		try:
			self.executeCommand(self.objectName, "endPositionRequest")
		except ApiError as e:
			print(e)
	def cmdSetOutput(self, pin, on):
		try:
			parameters = [pin, on]
